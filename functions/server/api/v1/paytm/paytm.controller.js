...
const initiatePayment = (req, res) => {
  try {
    ...
    params['CALLBACK_URL'] = `${appConfig.hostUrl}/api/v1/paytm/callback`;
    //'http://localhost:5000/paytm-integration-a80b8/us-central1/api/api/v1/paytm/callback';
   // 'https://us-central1-paytm-integration-a80b8.cloudfunctions.net/api/api/v1/paytm/callback';
    //`/api/v1/paytm/callback`;
    params['EMAIL'] = queryParams.email;
  
    checksum_lib.genchecksum(params, PaytmConfig.key, function (err, checksum) {
    if(err){
	log.error("error in initiatePayment checksum_lib.genchecksum : ",err);
    	throw new Error(err) ;
    	}
...
const callback = (req, res) => {
  try {
    ...
    checksum_lib.genchecksum(params, PaytmConfig.key, function (err, checksum) {
	if(err){
	log.error("error callback in checksum_lib.genchecksum : ",err);
    	throw new Error(err) ;
    	}
